...
export function ensureFinanceSchema() {
  ...
  ready = (async () => {
    const statements = [
      ...
      `CREATE UNIQUE INDEX IF NOT EXISTS installment_labels_owner_group_idx ON installment_labels (owner_id, group_id)`,
      ...
      `CREATE UNIQUE INDEX IF NOT EXISTS establishment_rules_owner_match_idx ON establishment_rules (owner_id, match_text)`,
      `CREATE TABLE IF NOT EXISTS notification_app_rules (
        id TEXT PRIMARY KEY NOT NULL,
        owner_id TEXT NOT NULL,
      ...
      `CREATE UNIQUE INDEX IF NOT EXISTS notification_app_rules_owner_app_idx ON notification_app_rules (owner_id, source_app)`,
      `CREATE TABLE IF NOT EXISTS pending_auto_transactions (
        id TEXT PRIMARY KEY NOT NULL,
        owner_id TEXT NOT NULL,
        source_app TEXT NOT NULL,
      ...
      `CREATE INDEX IF NOT EXISTS pending_auto_transactions_owner_status_idx ON pending_auto_transactions (owner_id, review_status)`,
    ...
    if (!transactionColumns.has("auto_imported")) await env.DB.prepare("ALTER TABLE transactions ADD COLUMN auto_imported INTEGER NOT NULL DEFAULT 0").run();
    if (!transactionColumns.has("review_status")) await env.DB.prepare("ALTER TABLE transactions ADD COLUMN review_status TEXT NOT NULL DEFAULT 'confirmed'").run();
    if (!transactionColumns.has("source_app")) await env.DB.prepare("ALTER TABLE transactions ADD COLUMN source_app TEXT").run();
    if (!transactionColumns.has("raw_text")) await env.DB.prepare("ALTER TABLE transactions ADD COLUMN raw_text TEXT").run();
    if (!transactionColumns.has("confidence")) await env.DB.prepare("ALTER TABLE transactions ADD COLUMN confidence REAL").run();
    await env.DB.prepare("CREATE INDEX IF NOT EXISTS transactions_owner_review_idx ON transactions (owner_id, review_status)").run();
    ...
    if (!accountColumns.has("exclude_from_totals")) await env.DB.prepare("ALTER TABLE accounts ADD COLUMN exclude_from_totals INTEGER NOT NULL DEFAULT 0").run();
