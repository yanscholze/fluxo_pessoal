...
import { clearNotificationBridgeCredentials, syncNotificationBridgeCredentials } from "./notification-bridge";

// Set EXPO_PUBLIC_API_ORIGIN when building (e.g. in a .env file read by
// `expo start`/`eas build`) to point the app at your own deployment. Falls
// back to the original OpenAI-hosted URL if unset.
const API_ORIGIN = process.env.EXPO_PUBLIC_API_ORIGIN || "https://fluxo-pessoal.yan-scholze.chatgpt.site";
const keys = {
...
export async function connectWithBrowser() {
  ...
  await syncNotificationBridgeCredentials(API_ORIGIN, callback.token);
...
export async function clearSession() {
  ...
  await clearNotificationBridgeCredentials();
}
