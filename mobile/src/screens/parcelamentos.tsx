  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: palette.canvas }} edges={["top"]}>
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 12, paddingBottom: 40, gap: 14 }} refreshControl={<RefreshControl refreshing={remoto.carregando} onRefresh={remoto.recarregar} tintColor={palette.accent} />}>
        ...
        {dados?.commitment.length ? <View style={{ borderRadius: radius.lg, borderWidth: 1, borderColor: palette.line, backgroundColor: palette.surface, padding: 16 }}><View style={{ flexDirection: "row", alignItems: "center" }}><Texto style={[type.bodyStrong, { color: palette.ink, flex: 1 }]}>Comprometimento mensal</Texto><View style={{ flexDirection: "row", gap: 9 }}><Legenda cor={palette.lineStrong} texto="Total" /><Legenda cor={palette.accent} texto="Pago" /></View></View><View style={{ height: 106, flexDirection: "row", alignItems: "flex-end", gap: 7, marginTop: 15 }}>{dados.commitment.slice(0, 7).map((mes, indice) => { const altura = Math.max(4, (mes.amountCents / maximo) * 80); return <View key={mes.competence} style={{ flex: 1, alignItems: "center" }}><View style={{ height: 80, width: "100%", flexDirection: "row", justifyContent: "center", alignItems: "flex-end", gap: 3 }}><View style={{ width: "27%", height: altura, borderRadius: 4, backgroundColor: palette.lineStrong }} /><View style={{ width: "27%", height: Math.max(4, altura * limite(dados.totals.percentPaid) / 100), borderRadius: 4, backgroundColor: indice === 0 ? "#9b82ff" : palette.accent }} /></View><Texto style={[type.caption, { color: indice === 0 ? palette.ink : palette.inkSubtle, marginTop: 6 }]}>{competence(mes.competence as never).slice(0, 3)}</Texto></View>; })}</View></View> : null}
...
function Legenda({ cor, texto }: { cor: string; texto: string }) { const palette = usePalette(); return <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}><View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: cor }} /><Texto style={[type.caption, { color: palette.inkSubtle }]}>{texto}</Texto></View>; }
