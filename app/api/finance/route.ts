...
import { accounts, cards, categories, establishmentRules, installmentLabels, notificationAppRules, pendingAutoTransactions, recurringEntries, rewardRedemptions, transactions, trips } from "../../../db/schema";
import { normalizeEstablishment } from "../../../lib/auto-transactions";
...
function transactionJson(row: typeof transactions.$inferSelect): FinanceTransaction {
  return {
    ...
    autoImported: Boolean(row.autoImported), reviewStatus: row.reviewStatus as FinanceTransaction["reviewStatus"],
    sourceApp: row.sourceApp ?? undefined, rawText: row.rawText ?? undefined, confidence: row.confidence ?? undefined,
  };
}
...
export async function financeGetForOwner(request: Request, ownerId: string) {
  try {
    ...
    const [accountRows, categoryRows, cardRows, tripRows, recurringRows, redemptionRows, installmentLabelRows, establishmentRuleRows, appRuleRows, pendingRows] = await Promise.all([
      ...
      db.select().from(installmentLabels).where(eq(installmentLabels.ownerId, ownerId)),
      db.select().from(establishmentRules).where(eq(establishmentRules.ownerId, ownerId)),
      db.select().from(notificationAppRules).where(eq(notificationAppRules.ownerId, ownerId)),
      db.select().from(pendingAutoTransactions).where(eq(pendingAutoTransactions.ownerId, ownerId)).orderBy(desc(pendingAutoTransactions.createdAt)).limit(200),
    ]);
    ...
    return Response.json({
      accounts: accountRows.map((row) => ({ id: row.id, name: row.name, institution: row.institution, currency: row.currency, kind: row.kind, balance: row.balanceCents / 100, goal: row.goalCents / 100, monthlyYieldPercent: row.monthlyYieldBasisPoints / 100, fixed: row.fixed, excludeFromTotals: row.excludeFromTotals, color: row.color })),
      ...
      installmentLabels: Object.fromEntries(installmentLabelRows.map((row) => [row.groupId, row.label])),
      establishmentRules: establishmentRuleRows.map((row) => ({ id: row.id, matchText: row.matchText, category: row.category })),
      notificationAppRules: appRuleRows.map((row) => ({ id: row.id, sourceApp: row.sourceApp, action: row.action as "ignore" | "allow", accountId: row.accountId ?? undefined, cardId: row.cardId ?? undefined, defaultCategory: row.defaultCategory ?? undefined })),
      pendingAutoTransactions: pendingRows.map((row) => ({
        id: row.id, sourceApp: row.sourceApp, rawText: row.rawText, confidence: row.confidence, description: row.description,
        amount: row.amountCents / 100, type: row.type as TransactionType, date: row.occurredAt,
        suggestedCategory: row.suggestedCategory ?? undefined, suggestedAccount: row.suggestedAccount ?? undefined, cardId: row.cardId ?? undefined,
        reviewStatus: row.reviewStatus as "new" | "ignored" | "duplicate", ignoreReason: row.ignoreReason ?? undefined, createdAt: row.createdAt,
      })),
...
export async function financePostForOwner(request: Request, ownerId: string) {
  try {
    ...
    const payload = await request.json() as {
      account?: { id?: string; name?: string; institution?: string; currency?: string; kind?: string; balance?: number; goal?: number; monthlyYieldPercent?: number; fixed?: boolean; excludeFromTotals?: boolean; color?: string };
      ...
      installmentLabel?: { groupId?: string; label?: string };
      transactionReview?: { id?: string; action?: "confirm" | "ignore" | "duplicate" };
      establishmentRule?: { matchText?: string; category?: string };
      notificationAppRule?: { sourceApp?: string; action?: "ignore" | "allow"; accountId?: string; cardId?: string; defaultCategory?: string };
      pendingAutoTransaction?: { id?: string; action?: "confirm" | "ignore" | "duplicate"; overrides?: { description?: string; category?: string; account?: string; amount?: number; date?: string } };
      ...
      transaction?: { id?: string; description?: string; category?: string; account?: string; destinationAccount?: string; date?: string; amount?: number; type?: string; paymentMethod?: PaymentMethod; cardId?: string; tripId?: string; invoiceMonth?: string; installments?: string; installmentGroupId?: string; importBatchId?: string; importBatchName?: string; importBatchMonth?: string; importedAt?: string; status?: string; source?: string; fingerprint?: string; rewardPoints?: number; rewardCashback?: number; rewardUsdRate?: number; sourceApp?: string; rawText?: string; confidence?: number };
    };
    const db = getDb();

    if (payload.transactionReview) {
      const { id, action } = payload.transactionReview;
      if (!id || !["confirm", "ignore", "duplicate"].includes(action ?? "")) return Response.json({ error: "Ação de revisão inválida" }, { status: 400 });
      const row = (await db.select().from(transactions).where(and(eq(transactions.id, id), eq(transactions.ownerId, ownerId))).limit(1))[0];
      if (!row) return Response.json({ error: "Lançamento não encontrado" }, { status: 404 });
      const now = new Date().toISOString();
      if (action === "confirm") {
        await db.update(transactions).set({ reviewStatus: "confirmed", updatedAt: now }).where(eq(transactions.id, id));
      } else {
        // "ignore"/"duplicate": some money already moved when the transaction was created, so we
        // soft-delete it (reverting the balance impact) instead of just flagging it.
        await applyAccountDeltas(ownerId, row, -1);
        await db.update(transactions).set({ reviewStatus: action, deletedAt: now, updatedAt: now }).where(eq(transactions.id, id));
      }
      const updated = (await db.select().from(transactions).where(eq(transactions.id, id)).limit(1))[0];
      return Response.json({ transaction: transactionJson(updated) });
    }

    if (payload.establishmentRule) {
      const matchText = normalizeEstablishment(payload.establishmentRule.matchText ?? "");
      const category = payload.establishmentRule.category?.trim();
      if (!matchText || !category) return Response.json({ error: "Regra inválida" }, { status: 400 });
      const id = `${ownerId}:est:${matchText}`;
      await db.insert(establishmentRules).values({ id, ownerId, matchText, category, createdAt: new Date().toISOString() })
        .onConflictDoUpdate({ target: establishmentRules.id, set: { category } });
      return Response.json({ establishmentRule: { id, matchText, category } });
    }

    if (payload.notificationAppRule) {
      const sourceApp = payload.notificationAppRule.sourceApp?.trim();
      if (!sourceApp) return Response.json({ error: "Regra inválida" }, { status: 400 });
      const action = payload.notificationAppRule.action === "allow" ? "allow" : "ignore";
      const id = `${ownerId}:app:${sourceApp}`;
      await db.insert(notificationAppRules).values({
        id, ownerId, sourceApp, action,
        accountId: payload.notificationAppRule.accountId?.trim() || null,
        cardId: payload.notificationAppRule.cardId?.trim() || null,
        defaultCategory: payload.notificationAppRule.defaultCategory?.trim() || null,
        createdAt: new Date().toISOString(),
      }).onConflictDoUpdate({ target: notificationAppRules.id, set: {
        action,
        accountId: payload.notificationAppRule.accountId?.trim() || null,
        cardId: payload.notificationAppRule.cardId?.trim() || null,
        defaultCategory: payload.notificationAppRule.defaultCategory?.trim() || null,
      } });
      return Response.json({ notificationAppRule: { id, sourceApp, action } });
    }

    if (payload.pendingAutoTransaction) {
      const { id, action } = payload.pendingAutoTransaction;
      if (!id || !["confirm", "ignore", "duplicate"].includes(action ?? "")) return Response.json({ error: "Ação inválida" }, { status: 400 });
      const pending = (await db.select().from(pendingAutoTransactions).where(and(eq(pendingAutoTransactions.id, id), eq(pendingAutoTransactions.ownerId, ownerId))).limit(1))[0];
      if (!pending) return Response.json({ error: "Item não encontrado" }, { status: 404 });
      if (action !== "confirm") {
        await db.update(pendingAutoTransactions).set({ reviewStatus: action }).where(eq(pendingAutoTransactions.id, id));
        return Response.json({ ok: true });
      }
      const now = new Date().toISOString();
      const newId = `auto-${pending.id}`;
      const overrides = payload.pendingAutoTransaction.overrides ?? {};
      const values = {
        description: (overrides.description ?? pending.description).trim(),
        category: overrides.category?.trim() || pending.suggestedCategory || "Outros",
        account: overrides.account?.trim() || pending.suggestedAccount || "Nubank",
        destinationAccount: null, occurredAt: overrides.date ?? pending.occurredAt,
        amountCents: overrides.amount != null ? Math.round(Number(overrides.amount) * 100) : pending.amountCents,
        type: pending.type, paymentMethod: pending.cardId ? "credit" : "other",
        cardId: pending.cardId, tripId: null, invoiceMonth: null,
        rewardPointsMilli: null, rewardCashbackCents: null, rewardUsdRateMicros: null,
        installments: null, installmentGroupId: null,
        importBatchId: null, importBatchName: null, importBatchMonth: null, importedAt: null,
        status: "confirmed", source: "notification",
        fingerprint: null, version: 1, deletedAt: null, deviceId: "notification", lastMutationId: null, updatedAt: now,
        autoImported: 1, reviewStatus: "confirmed", sourceApp: pending.sourceApp, rawText: pending.rawText, confidence: pending.confidence,
      };
      await db.insert(transactions).values({ id: newId, ownerId, ...values });
      await applyAccountDeltas(ownerId, values);
      await db.update(pendingAutoTransactions).set({ reviewStatus: "confirmed" }).where(eq(pendingAutoTransactions.id, id));
      const saved = (await db.select().from(transactions).where(eq(transactions.id, newId)).limit(1))[0];
      return Response.json({ transaction: transactionJson(saved) });
    }

    if (payload.installmentLabel) {
      const groupId = payload.installmentLabel.groupId?.trim();
      const label = payload.installmentLabel.label?.trim().slice(0, 80) ?? "";
      if (!groupId) return Response.json({ error: "Parcelamento inválido" }, { status: 400 });
      const now = new Date().toISOString();
      if (!label) {
        await db.delete(installmentLabels).where(and(eq(installmentLabels.ownerId, ownerId), eq(installmentLabels.groupId, groupId)));
        return Response.json({ installmentLabel: { groupId, label: null } });
      }
      const id = `${ownerId}:${groupId}`;
      await db.insert(installmentLabels).values({ id, ownerId, groupId, label, updatedAt: now })
        .onConflictDoUpdate({ target: installmentLabels.id, set: { label, updatedAt: now } });
      return Response.json({ installmentLabel: { groupId, label } });
    }
    ...
    const now = new Date().toISOString();
    const source = transactionType === "transfer" ? "account-transfer" : ["import", "recurring", "invoice-payment", "notification"].includes(item.source ?? "") ? String(item.source) : previous?.source === "notification" ? "notification" : "manual";
    const wasAutoImported = Boolean(previous?.autoImported);
    const isNewNotification = !previous && source === "notification";
    const autoImportedFlag = wasAutoImported || isNewNotification;
    const reviewStatus = isNewNotification ? "new" : wasAutoImported && previous?.reviewStatus !== "edited" ? "edited" : previous?.reviewStatus ?? "confirmed";
    const values = {
      ...
      autoImported: autoImportedFlag ? 1 : 0, reviewStatus, sourceApp: previous?.sourceApp ?? (isNewNotification ? item.sourceApp?.trim() || null : null), rawText: previous?.rawText ?? (isNewNotification ? item.rawText?.trim() || null : null), confidence: previous?.confidence ?? (isNewNotification ? item.confidence ?? null : null),
    };
