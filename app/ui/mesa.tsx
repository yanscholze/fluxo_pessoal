...
export function MetricTile({
  ...
  return (
    <section className="glass-panel flex min-h-[112px] items-center gap-3 p-[18px]">
      <div className={join("grid size-[42px] shrink-0 place-items-center rounded-[13px] ring-1", TONS[tom])}>
        {icone}
      </div>
      <div className="min-w-0">
        <p className="metric-label">{rotulo}</p>
        <div className="mt-0.5 flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
          <p className="tabular text-figure font-bold text-ink">{valor}</p>
          {variacao}
        </div>
        {apoio ? <p className="mt-0.5 text-[11px] leading-snug text-ink-subtle">{apoio}</p> : null}
      </div>
    </section>
  );
}
