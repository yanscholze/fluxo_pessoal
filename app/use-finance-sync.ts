...
export function useFinanceSync(ownerKey: string) {
  ...
  const [installmentLabels, setInstallmentLabels] = useState<Record<string, string>>({});
  const [establishmentRules, setEstablishmentRules] = useState<EstablishmentRule[]>([]);
  const [notificationAppRules, setNotificationAppRules] = useState<NotificationAppRule[]>([]);
  const [pendingAutoTransactions, setPendingAutoTransactions] = useState<PendingAutoTransaction[]>([]);
  ...
  const applySnapshot = useCallback((snapshot: FinanceSnapshot, queued: FinanceTransaction[] = []) => {
    ...
    setInstallmentLabels(snapshot.installmentLabels ?? {});
    setEstablishmentRules(snapshot.establishmentRules ?? []);
    setNotificationAppRules(snapshot.notificationAppRules ?? []);
    setPendingAutoTransactions(snapshot.pendingAutoTransactions ?? []);
  ...
  const saveInstallmentLabel = useCallback(async (groupId: string, label: string) => {
    setStatus("saving");
    try {
      const payload = await postFinance<{ installmentLabel: { groupId: string; label: string | null } }>({ installmentLabel: { groupId, label } });
      setInstallmentLabels((current) => {
        const next = { ...current };
        if (payload.installmentLabel.label) next[groupId] = payload.installmentLabel.label;
  ...
  const reviewTransaction = useCallback(async (id: string, action: "confirm" | "ignore" | "duplicate") => {
    setStatus("saving");
    try {
      const payload = await postFinance<{ transaction: FinanceTransaction }>({ transactionReview: { id, action } });
      if (action === "confirm") setTransactions((current) => current.map((item) => item.id === id ? payload.transaction : item));
      else setTransactions((current) => current.filter((item) => item.id !== id));
      setStatus("synced");
    } catch (error) { setStatus("error"); throw error; }
  }, []);

  const reviewPendingAutoTransaction = useCallback(async (id: string, action: "confirm" | "ignore" | "duplicate", overrides?: { description?: string; category?: string; account?: string; amount?: number; date?: string }) => {
    setStatus("saving");
    try {
      const payload = await postFinance<{ transaction?: FinanceTransaction }>({ pendingAutoTransaction: { id, action, overrides } });
      setPendingAutoTransactions((current) => current.filter((item) => item.id !== id));
      if (payload.transaction) setTransactions((current) => [payload.transaction!, ...current]);
      setStatus("synced");
    } catch (error) { setStatus("error"); throw error; }
  }, []);

  const saveEstablishmentRule = useCallback(async (matchText: string, category: string) => {
    setStatus("saving");
    try {
      const payload = await postFinance<{ establishmentRule: EstablishmentRule }>({ establishmentRule: { matchText, category } });
      setEstablishmentRules((current) => [...current.filter((item) => item.matchText !== payload.establishmentRule.matchText), payload.establishmentRule]);
      setStatus("synced");
    } catch (error) { setStatus("error"); throw error; }
  }, []);

  const saveNotificationAppRule = useCallback(async (sourceApp: string, action: "ignore" | "allow", options?: { accountId?: string; cardId?: string; defaultCategory?: string }) => {
    setStatus("saving");
    try {
      const payload = await postFinance<{ notificationAppRule: NotificationAppRule }>({ notificationAppRule: { sourceApp, action, ...options } });
      setNotificationAppRules((current) => [...current.filter((item) => item.sourceApp !== payload.notificationAppRule.sourceApp), { ...payload.notificationAppRule, ...options }]);
      setStatus("synced");
    } catch (error) { setStatus("error"); throw error; }
  }, []);
  ...
  return {
    transactions, accounts, categories, cards, trips, rewardRedemptions, salaryRule, benefitRule, recurringRules, installmentLabels, exchangeRate, status,
    establishmentRules, notificationAppRules, pendingAutoTransactions,
    ...
    saveRecurringRule, saveTrip, removeTrip, payInvoice, redeemReward, confirmSalary, saveInstallmentLabel, retrySync: flushQueue,
    reviewTransaction, reviewPendingAutoTransaction, saveEstablishmentRule, saveNotificationAppRule,
  };
}
